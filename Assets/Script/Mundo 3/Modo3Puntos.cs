...
public class Modo3Puntos : MonoBehaviour {
	...
	void Start(){
		...
		Nivel = PlayerPrefs.GetInt("level3");
		switch (Nivel){
		case 1 :
			Id1="1";
			Id2="2";
	...
	void Update(){
		...
		if(jugar){
			if(!FingerEventsSamplePart2.Levanto){
				...
				if(id1Bool && id2Bool && id3Bool && id4Bool && id5Bool && id6Bool && id7Bool && id8Bool && id9Bool){
					if(Final == FinalTemp){
						Debug.Log("Ganaste");
						int Level = PlayerPrefs.GetInt ("level3");
						Level++;
						PlayerPrefs.SetInt ("level3", Level);
